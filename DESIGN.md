...
- **Snapshot** — the figure display. Compact stacked header (kicker left, as-of date right, rule at the foot), then four cells held apart by `gap`, each a `figure` number over a `label` caption, with a derivation note beneath. Labels sit on one line. It sits **below** the firm introduction, never above it — the visitor meets the firm before its numbers.
